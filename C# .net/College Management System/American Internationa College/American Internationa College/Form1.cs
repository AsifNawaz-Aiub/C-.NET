            ...
            if(dt.Rows.Count>0)
            {
                for(int i=0; i<dt.Rows.Count;i++)
                {
                    if(dt.Rows[i][0].ToString()== userName && dt.Rows[i][1].ToString()== password)
                    {
                        ...
                        if (dt.Rows[i][2].ToString() == "Principal")
                        {
                            this.Hide();
                            Principle principle = new Principle(txtUserName.Text);
                            principle.Show();
                        }
                        if (dt.Rows[i][2].ToString() == "Teacher")
                        {
                            this.Hide();
                            Teacher tr = new Teacher(txtUserName.Text);
                            tr.Show();
                        }
                        if (dt.Rows[i][2].ToString() == "Student")
                        {
                            this.Hide();
                            Student st = new Student(txtUserName.Text);
                            st.Show();
                        }
                    }
                }
